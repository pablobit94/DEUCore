        ...
        if (ID < SPAWN) // no args
        {
            if (ID >= DELET && ID <= GOTO) // has target (needs retrieve gameobject)
            {
                ...
                else
                {
                    ...
                    case X: GOMove::MoveGameObject(player, player->GetPositionX(), y, z, o, lowguid);       break;
                    case Y: GOMove::MoveGameObject(player, x, player->GetPositionY(), z, o, lowguid);       break;
                    case Z: GOMove::MoveGameObject(player, x, y, player->GetPositionZ(), o, lowguid);       break;
                    case O: GOMove::MoveGameObject(player, x, y, z, player->GetOrientation(), lowguid);     break;
                    case RESPAWN: GOMove::SpawnGameObject(player, x, y, z, o, target->GetEntry());                   break;
                    case GOTO:
                    {
                        // stop flight if need
        ...
        else if (ARG && ID >= SPAWN)
        {
            if (ID >= NORTH)
            {
                ...
                else
                {
                    ...
                    switch (ID)
                    {
                    case NORTH: GOMove::MoveGameObject(player, x + ((float)ARG / 100), y, z, o, lowguid);                            break;
                    case EAST: GOMove::MoveGameObject(player, x, y - ((float)ARG / 100), z, o, lowguid);                             break;
                    case SOUTH: GOMove::MoveGameObject(player, x - ((float)ARG / 100), y, z, o, lowguid);                            break;
                    case WEST: GOMove::MoveGameObject(player, x, y + ((float)ARG / 100), z, o, lowguid);                             break;
                    case NORTHEAST: GOMove::MoveGameObject(player, x + ((float)ARG / 100), y - ((float)ARG / 100), z, o, lowguid);   break;
                    case SOUTHEAST: GOMove::MoveGameObject(player, x - ((float)ARG / 100), y - ((float)ARG / 100), z, o, lowguid);   break;
                    case SOUTHWEST: GOMove::MoveGameObject(player, x - ((float)ARG / 100), y + ((float)ARG / 100), z, o, lowguid);   break;
                    case NORTHWEST: GOMove::MoveGameObject(player, x + ((float)ARG / 100), y + ((float)ARG / 100), z, o, lowguid);   break;
                    case UP: GOMove::MoveGameObject(player, x, y, z + ((float)ARG / 100), o, lowguid);                               break;
                    case DOWN: GOMove::MoveGameObject(player, x, y, z - ((float)ARG / 100), o, lowguid);                             break;
                    case RIGHT: GOMove::MoveGameObject(player, x, y, z, o - ((float)ARG / 100), lowguid);                            break;
                    case LEFT: GOMove::MoveGameObject(player, x, y, z, o + ((float)ARG / 100), lowguid);                             break;
                    }
                }
            }
            else
            {
                ...
                case SPAWN:
                {
                    if (GOMove::SpawnGameObject(player, player->GetPositionX(), player->GetPositionY(), player->GetPositionZ(), player->GetOrientation(), ARG))
    ...
    class GOMove_spellscript : public SpellScript
    {
        ...
        void HandleAfterCast()
        {
            ...
            if (uint32 entry = GOMove::Store.SpawnQueGet(player->GetGUID()))
                GOMove::SpawnGameObject(player, summonPos->GetPositionX(), summonPos->GetPositionY(), summonPos->GetPositionZ(), player->GetOrientation(), entry);
        }
...
class GOMove_spell_place2 : public SpellScriptLoader
{
public:
    GOMove_spell_place2() : SpellScriptLoader("GOMove_spell_place2") { }

    class GOMove_spellscript2 : public SpellScript
    {
        PrepareSpellScript(GOMove_spellscript2);
        ...
        void HandleAfterCast()
        {
            ...
            if (!summonPos)
                return;
            if (uint32 entry = player->GetMaxHealth()) {
                Map* map = player->GetMap();
                GameObjectTemplate const* objectInfo = sObjectMgr->GetGameObjectTemplate(entry);
                if (!objectInfo) {
                    return;
                }
                GameObject* object = GameObject::CreateGameObject(objectInfo->entry, map, summonPos->GetPosition(), QuaternionData::fromEulerAnglesZYX(player->GetOrientation(), 0.0f, 0.0f), 255, GO_STATE_READY);
        ...
        void Register() override
        {
            AfterCast += SpellCastFn(GOMove_spellscript2::HandleAfterCast);
        }
    };
    SpellScript* GetSpellScript() const override
    {
        return new GOMove_spellscript2();
    }
};
