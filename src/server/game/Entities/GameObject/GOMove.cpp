...
GameObject * GOMove::SpawnGameObject(Player* player, float x, float y, float z, float o, uint32 entry)
{
    if (!player || !entry)
        return nullptr;
...
GameObject * GOMove::MoveGameObject(Player* player, float x, float y, float z, float o, ObjectGuid::LowType lowguid)
{
    if (!player)
        return nullptr;
